  ...
  getNotes() {
    return {
      ...
      nodes: [
        ...
        { id: 78, label: 'Input Validation', category: 'Secure Coding', info: 'The essential programming practice of checking, filtering, and sanitizing user input before processing it to prevent injection attacks (SQLi, XSS).' },
        { id: 79, label: 'Memory Safety', category: 'Programming', info: 'A concept ensuring that code accesses only the memory locations it is authorized to access. Languages like Rust and C# offer built-in safety; C/C++ requires manual handling.' },
        { id: 80, label: 'Integer Overflow/Underflow', category: 'Programming Vulnerability', info: 'Software vulnerabilities that occur when an arithmetic operation attempts to create a numeric value that is too large or too small to be represented by the available storage space (critical in blockchain/smart contract security).' },
        { id: 81, label: 'Race Condition', category: 'Concurrency/Vulnerability', info: 'A critical, time-dependent flaw where the output of a process depends on the sequence or timing of other uncontrollable events, often exploited for privilege escalation (e.g., "check-then-act" flaws).' },
        { id: 82, label: 'Exploit Primitives', category: 'Exploitation Theory', info: 'Fundamental, low-level capabilities gained during an attack, such as an Arbitrary Read (read any memory location) or Arbitrary Write (write to any memory location).' },
        { id: 83, label: 'Kernel Exploitation', category: 'OS/Exploitation', info: 'Attacks targeting the operating system kernel (privileged ring 0) to gain the highest level of control, often involving vulnerabilities in device drivers or syscall handling.' },
        { id: 84, label: 'Side-Channel Attack', category: 'Cryptanalysis', info: 'An attack based on information gained from the implementation of a computer system, rather than weaknesses in the algorithm itself (e.g., measuring power consumption or timing of cryptographic operations).' },
        { id: 85, label: 'Hardware Root of Trust', category: 'Hardware Security', info: 'The first immutable code that executes upon system startup, responsible for verifying the integrity of the subsequent boot code (e.g., in TPMs or secure boot).' },
        { id: 86, label: 'Sandbox Escapes', category: 'Advanced Exploitation', info: 'Techniques used to break out of a restrictive environment (such as a browser, container, or VM) to gain control of the host operating system.' },
        { id: 87, label: 'ASLR', category: 'OS Defense', info: 'Address Space Layout Randomization: A memory-protection technique that randomly arranges the positions of key data areas in a process\'s memory space to make exploitation (ROP) difficult.' },
        { id: 88, label: 'DEP / NX bit', category: 'OS Defense', info: 'Data Execution Prevention / No-Execute bit: A hardware/software feature that marks certain areas of memory as non-executable, preventing the execution of code from data areas (a defense against buffer overflows).' },
        { id: 89, label: 'Hypervisor', category: 'Virtualization', info: 'The software/firmware that creates and runs virtual machines (VMs). Type 1 (Bare-Metal) is used in data centers; Type 2 (Hosted) is used on desktops.' },
        { id: 90, label: 'VM Escape', category: 'Virtualization Security', info: 'A type of exploit that allows an attacker to gain control of the host operating system (Hypervisor) from within a guest virtual machine.' },
        { id: 91, label: 'Fileless Malware', category: 'Malware Evasion', info: 'Malicious software that operates solely in the system\'s volatile memory (RAM), avoiding detection by traditional file-based antivirus solutions (often uses PowerShell or WMI).' },
        { id: 92, label: 'Registry', category: 'Windows Core', info: 'The central hierarchical database used to store information necessary to configure the system for one or more users, applications, and hardware devices (critical for Windows forensic and malware analysis).' },
        { id: 93, label: 'Cyber Kill Chain', category: 'Methodology', info: 'A model describing the stages of a cyber attack from initial reconnaissance to objective completion (Lockheed Martin framework).' },
        { id: 94, label: 'Diamond Model', category: 'Methodology', info: 'Diamond Model of Intrusion Analysis: A structured methodology for analyzing cyber security incidents by mapping the four core features: Adversary, Capability, Infrastructure, and Victim.' },
        { id: 95, label: 'Threat Intelligence', category: 'Intelligence', info: 'The collected and analyzed information about potential threats and adversaries that can be used to mitigate harmful events. Tiers: Strategic, Operational, Tactical.' },
        { id: 96, label: 'Digital Elicitation', category: 'OSINT/Social Eng.', info: 'Techniques used in OSINT to subtly draw information from a target online without direct, suspicious inquiry (a form of social engineering).' },
        { id: 97, label: 'Honeypot / Honeynet', category: 'Technology/Defense', info: 'A deliberately vulnerable system (Honeypot) or network (Honeynet) designed to attract and trap attackers to study their TTPs without risk to the production network.' }
      ]
    };
  }
}
